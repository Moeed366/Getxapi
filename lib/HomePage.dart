      ...
      GetBuilder<alpha>(
        builder: (cont) {
          return  cont.list.length==0?Center(child: CircularProgressIndicator(),):Column(
            children: [
              Expanded(
                child: ListView.builder(itemCount: cont.list.length,
                    itemBuilder: (BuildContext context, int index){
                  return Container(height: 100,width: 300,
                    child: ListTile(
                     title: Text(cont.list[index].name.toString(),),
                      trailing: Container(width: 150,height: 150,
                        child: Row(
                          children: [
                            FloatingActionButton(onPressed:() {},child: Icon(Icons.add,color: Colors.black,),),
                            Container(height:20,width:20,child: Text(cont.list[index].quantity.toString(),)),
